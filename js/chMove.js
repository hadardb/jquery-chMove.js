class chMove{
    constructor(parent,minWidth,minHeight,isAllChange){
        ...
        this.noBox=null
        this.clickDiv=null
        this.clickNoDiv=null
        this.keepShowDiv=null
        this.createParent()
    ...
    parentMouseOver(){
        $(this.parent).on('mouseover', event =>{
            ...
            if(thisDiv!=this.parent[0]&&(this.isAllChange || $(thisDiv).attr('data-move'))){
                ...
                $(this.noBox).children().each((index,element)=>{
                ...
                if(isCreate){
                    ...
                    this.minWidth=$(thisDiv).attr('data-minWidth') || this.minWidth
                    this.minHeight=$(thisDiv).attr('data-minHeight') || this.minHeight
    ...
    chMoveBoxMouseDown(){
        ...
        $(that.noBox).on('mousedown', function(event) {
            ...
            if($(square).hasClass('chMove_box_li')){
				...
				`)
                that.clickDiv=that.getBoxDiv($(square))
                that.clickNoDiv=$(square)
    ...
    hideMoveBox(){
        $(document).on('mousedown',(event) =>{
            ...
            event.stopPropagation()
            var square=event.target || event.srcElement
            var keepShow=this.keepShowDiv || `($(square).prop('tagName')=='INPUT' || 
			$(square).prop('tagName')=='BUTTON' || $(square).prop('contenteditable')==true)`
            if(eval(keepShow)){
                return
            }
    ...
    //设置点击网页时哪些元素不隐藏边框
    setkeepShow(text){
        this.keepShowDiv=text
    }
    synData(){
        let {width,height,top,left,position} = getComputedStyle(this.clickDiv[0],null)
        this.clickNoDiv.css({'width':width,'height':height,'top':top,'left':left,'position':position})
    }
}
